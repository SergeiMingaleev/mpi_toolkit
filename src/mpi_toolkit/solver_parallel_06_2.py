...
#
# Реализация №5: Пытаемся распараллелить все вычисления, используя
...
#                на каждой такой строке процессов.
#
#                Данный алгоритм точно воспроизводит алгоритм №4
#                из примера 6.1, отличаясь только техническим исполнением.
#                Именно, вместо использования двух коммуникаторов `comm_row`
#                и `comm_col` внутри функции `conjugate_gradient_method()`,
#                мы используем один новый коммуникатор `comm_cart`,
#                построенный по декартовой топологии с сеткой процессов
#                размером `(num_row, num_col)` встроенной непосредственно
#                внутрь коммуникатора. При этом вместо метода `Allreduce()`,
#                вызываемого поочерёдно на `comm_row` и `comm_col`,
#                мы теперь вызываем в цикле метод `Sendrecv_replace()`.
# ------------------------------------------------------------------
# Эта реализация детально разбирается в примере
# mpi_toolkit/examples/Example-06-2.py
# ------------------------------------------------------------------
...
# =============================================================================
class SolverParallelBlock3(SolverBase):
    """
    ...
    def _cgm_mpi_part(self, A_part, b_part, x_part,
                      N_part, M_part, N,
                      comm_cart, num_row, num_col):
        """
        Параллельное решение системы линейных уравнений A*x = b
        методом сопряжённых градиентов.

        Реализация №5: Данный алгоритм точно воспроизводит алгоритм №4
                       из примера 6.1, отличаясь только техническим исполнением.
                       Именно, вместо использования двух коммуникаторов `comm_row`
                       и `comm_col` внутри функции `conjugate_gradient_method()`,
                       мы используем один новый коммуникатор `comm_cart`,
        ...
        :param comm_cart: Декартовый коммуникатор для связи всех MPI
                          процессов между собой.
        :param num_row:   Число строк в декартовом коммуникаторе.
        :param num_col:   Число столбцов в декартовом коммуникаторе.
        ...
        # Найдём индексы (rank) соседних процессов по вертикали
        # (вдоль оси строк) и горизонтали (вдоль оси столбцов):
        ...
        # Каждый процесс будет хранить свою часть
        ...
        # Объявим также переменную `dot_vec_temp` для хранения частичных
        # сумм скалярного произведения векторов, собранных на отдельных
        # процессах. Как и в примере 6.1, здесь мы будем делать скалярное
        # умножение векторов на всех процессах, так что создадим
        # такую переменную также на всех процессах:
        ...
        for s in range(1, N + 1):
            ...
            if s == 1:
                ...
                # Теперь нам нужно просуммировать полученные вектора по всем
                ...
                # В отличие от примера 6.1, где нужная нам операция выполнялась
                # вызовом метода `comm_row.Allreduce()` на коммуникаторах каждой
                # строки сетки процессов, используем в этот раз вызов метода
                # `comm_cart.Sendrecv_replace()` на декартовом коммуникаторе -
                # каждый вызов этого метода будет циклически менять `Ax_part_temp`
                # между процессами каждой строки - а мы уже вручную просуммируем
                # эти векторы независимо на каждом процессе для получения `Ax_part`:
                ...
                # NOTE: существенное отличие между таким суммированием и вызовом
                # метода `comm_row.Allreduce()` заключается в том, что вызов
                # `comm_row.Allreduce()` гарантировал, что каждый процесс получит
                # одну и ту же копию вектора `Ax_part`. Здесь же порядок слагаемых
                # при суммировании будет отличаться на каждом процессе - и если
                # порядок значений слагаемых сильно отличается друг от друга,
                # то и окончательная сумма может отличаться на разных процессах
                # из-за накопленной ошибки округления.
                # !!! И В ЭТОМ АЛГОРИТМЕ - ЭТО ИМЕННО НАШ СЛУЧАЙ !!!
                ...
                # процессам своего столбца сетки процессов.
            ...
            # часть `Ap_part` такого произведения.
            # Снова, в отличие от примера 6.1, используем вызов метода
            # `comm_cart.Sendrecv_replace()` вместо `comm_row.Allreduce()`
            # и суммируем вектора локально на каждом процессе (из-за
            # чего конечный результат может отличаться на каждом процессе,
            ...
            # ВАЖНО: последние команды пересылают *частичный* вектор
            #        с размером `M_part` соседним процессам - такая операция
    ...
    def _conjugate_gradient_method(self, A, b, x):
        ...
        # Особенность данного примера - использование коммуникатора
        # с декартовой топологией, в точности соответсвующей требуемому
        # блочному разбиению матрицы, состоящие из `num_row` строк блоков
        # и `num_col` столбцов блоков:

        comm_cart = self._comm.Create_cart(dims=(num_row, num_col),
                                           periods=(True, True),
                                           reorder=True)
        # ВАЖНО:
        # Использование аргумента `reorder=True` заставит MPI протестировать
        # нашу сеть процессов и перенумеровать процессы таким образом, чтобы
        # у нас получалась в среднем максимально отзывчивая (low latency) и
        # быстрая (high bandwidth) связь между ближайшими соседними процессами
        # по вертикали и горизонтали. Это позволит максимально эффективно
        # наложить требуемую сетку процессов на реальную топологию используемого
        # суперкомпьютера.

        # Номер текущего процесса (от 0 до P-1) в рамках нового декартового
        # коммуникатора (при использовании в команде выше аргумента `reorder=False`,
        # значения `rank` и `rank_cart` на каждом процессе будут совпадать):
        ...
        color_col = rank_cart % num_col
        comm_col = comm_cart.Split(color_col, rank_cart)
        ...
        color_row = rank_cart // num_col
        comm_row = comm_cart.Split(color_row, rank_cart)
        ...
        # print(f"comm_cart = {comm_cart}")
        # print(f"comm_cart.Get_size() = {comm_cart.Get_size()}")
        # print(f"comm_cart.Get_rank() = {comm_cart.Get_rank()}")
        ...
        # блочной матрицы:
        if rank_cart in procs_first_row:
        ...
        if rank_cart in procs_first_col:
        ...
        # print(f"Vector `x_part` on process {rank_cart} consists of {len(x_part)} elements:")
