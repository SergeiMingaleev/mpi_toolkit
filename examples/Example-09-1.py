...
def diagonals_preparation(u_m):
    ...
    b = 1.0 - alpha * (- 2 * eps_dt_dx2
                       + dt_2dx * (u_m[2:] - u_m[:-2])
                       + 3 * dt * u_m[1:-1]**2)
    c = -alpha * (eps_dt_dx2 + dt_2dx * u_m[1:-1])
...
# с помощью "неявной" разностной схемы. Решаем уравнение максимально
# быстро, используя "срезы" для numpy array операций и быструю решалку
# `scipy.linalg.solve_banded` для решения системы линейных уравнений
# с трёхдиагональной матрицей.
# Обратите внимание, что хотя функция `fast_pde_solution()` вызывается
# без аргументов, она переиспользует многие из введённых выше
# переменных как глобальные переменные (все такие переменные указаны
# в функции как `global`).
