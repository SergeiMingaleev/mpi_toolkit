...
#    ускорить вычисления в ~14 раз. Оба варианта при этом по максимуму
#    используют "срезы" в массивах библиотеки `numpy` вместо циклов
#    по индексу `n`.
#------------------------------------------------------------------
...
def pde_jacobian(u_m, t):
    ...
    (которая получается взятием производной от `f[n]` по `u_m[n-1]`),
    главной диагонали `b` (получается взятием производной
    от `f[n]` по `u_m[n]`), и верхней кодиагонали `c` (получается
    взятием производной от `f[n]` по `u_m[n+1]`).
