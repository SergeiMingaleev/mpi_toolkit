...
#    соответствующих смещений "displs*" внутри функций
...
#    Эти списки создаются на Шаге 3 кода ниже и определяют
#    распределение элементов вектора `u[m]` по всем процессам
#    MPI коммуникатора в виде частичных векторов `u_part`
#    и `u_part_aux`, соответственно.
...
# На каждом MPI процессе будем держать часть вектора решения
# `u[m]` в частичном векторе `u_part` с размером `N_part`
# (смотри комментарии выше про `N_part`):
u_part = np.empty(N_part, dtype=np.float64)

# На каждом MPI процессе будем держать часть вектора решения
# `u[m]` в частичном векторе `u_part_aux` с размером `N_part_aux`
# (смотри комментарии выше про `N_part_aux`):
