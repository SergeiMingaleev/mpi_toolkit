...
def auxiliary_arrays_determination(M, P):
    ...
    решения `u[m+1]` по всем процессам MPI коммуникатора, включая
    и процесс 0, в виде частичных векторов `u_part`.
    Наша задача при этом - разбросать вектор `u[m+1]` по всем
...
def auxiliary_arrays_from_0(P, rcounts, displs):
    ...
    вектора решения `u[m]` по всем процессам MPI коммуникатора, включая
    и процесс 0, в виде вспомогательных частичных векторов `u_part_aux`.
    Наша задача при этом - разбросать вектор `u[m+1]` по всем процессам
    ...
    :param rcounts: Список числа элементов, определяющий распределение
              элементов вектора решения `u[m+1]` по всем процессам MPI
    ...
    """
    # Сделаем хранилища для рассчитываемых списков:
    ...
    # И поскольку этот элемент добавляется справа, смещение остаётся нулевым:
    displs_from_0[0] = 0

    # Цикл по всем промежуточным процессам:
...
# частичным векторам `u_part` и `u_part_aux` во всех процессах
# MPI коммуникатора, включая и процесс 0.

# Списки числа элементов и смещений для частичных векторов
# `u_part` - тут мы просто максимально равномерно и без пересечений
# разбрасываем вектор `u[m+1]` по всем MPI процессам.
...
# в частичных векторах `u_part` по всем процессам
# в виде значений `N_part`:
...
# справа от элементов, включённых в частичные векторы `u_part`.
# Всю логику расчёта этих списков мы перенесём в отдельную
...
# Подготовим "хранилище" для `N_part_aux` на всех процессах:
N_part_aux = np.array(0, dtype=np.int32)

# Разбросаем теперь список числа элементов `rcounts_from_0`
# в частичных векторах `u_part_aux` по всем процессам
# в виде значений `N_part_aux`:
comm.Scatter([rcounts_from_0, 1, MPI.INT],
             [N_part_aux, 1, MPI.INT], root=0)

#------------------------------------------------------------------
# Шаг 4:
# Создаём массивы для хранения решения и задаём начальное условие.

# Считаем, что полное решение нашего ДУЧП накапливается в массиве
# `u[m, n]` только на процессе 0 (на остальных же процессах `u` - это
# пустой массив). Здесь `n` - это индекс по пространственной переменной `x`,
# а `m` - это индекс по моментам времени `t`. Доступ к элементу `u[m, n]`
# возможен также как к `u[m][n]` - при этом вектор `u[m]` соответствует
# решению ДУЧП в момент времени `m`.

if rank == 0:
    # Будем хранить полный массив `u` только на процессе 0:
    u = np.empty((M+1, N+1), dtype=np.float64)
    # Начальное условие:
    u[0, :] = u_init(x)
else:
    # На всех остальных процессах зададим `u` как пустой массив
    # с размером (M+1)x0 (вместо привычной уже инициализации его
    # значением `None`). Такой пустой массив создаётся для того,
    # чтобы формально можно было обратиться к его (пустым!)
    # векторам `u[m]` - это позволяет использовать более короткую
    # запись (без условных проверок) при организации вызовов
    # `comm.Scatterv()` и `comm.Gatherv()` внутри функций
    # `slow_pde_solution()` и `fast_pde_solution()`:
    u = np.empty((M+1, 0), dtype=np.float64)

# На каждом процессе, включая процесс 0, будем держать часть
# вектора решения `u[m+1]` в частичном векторе `u_part` с размером
# `N_part` (смотри комментарии выше про `N_part`):
u_part = np.empty(N_part, dtype=np.float64)

# На каждом процессе, включая процесс 0, будем держать часть
# вектора решения `u[m]` в частичном векторе `u_part_aux` с размером
# `N_part_aux` (смотри комментарии выше про `N_part_aux`):
