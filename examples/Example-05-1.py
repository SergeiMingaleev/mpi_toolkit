...
def conjugate_gradient_method(A_part, b_part, x_part,
    ...
    Также на каждом процессе первого столбца сетки процессов хранится
    свой кусочек `b_part` вектора `b`, а на каждом процессе первой
    строки сетки процессов хранится свой кусочек `x_part` вектора `x`.
    ...
    :param rank:   Номер текущего процесса в терминах общего коммуникатора.
    ...
    # первого столбца сетки процессов есть свой кусочек `b_part` вектора `b`.
    # Наконец, на каждом процессе первой строки сетки процессов есть свой
    # кусочек `x_part` вектора `x`, тогда как на всех остальных процессах
    # уже выделена память для нужного ему кусочка `x_part`.

    # Для удобства, введём список процессов из первой строки сетки процессов:
    ...
    # Каждый процесс из первой строки сетки процессов будет хранить
    # свою часть вспомогательных векторов `r` и `q` в массивах
    # `r_part` и `q_part`:
    if rank in procs_first_row:
        r_part = np.empty(N_part, dtype=np.float64)
        q_part = np.empty(N_part, dtype=np.float64)
    else:
        # На остальных процессах поставим просто заглушки:
        r_part = q_part = None
    ...
    # делать скалярное умножение векторов только на процессах первой
    # строки сетки процессов - поэтому нам достаточно хранить такие
    # переменные только на них:
    if rank in procs_first_row:
    ...
    for s in range(1, N + 1):
        ...
        if s == 1:
            ...
            # Сначала каждый процесс из первой строки сетки процессов
            # раздаст копию своей части `x_part` всем процессам
            # в пределах своего столбца сетки процессов:
            comm_col.Bcast([x_part, N_part, MPI.DOUBLE], root=0)
            # Теперь параллельно на каждом процессе можно сделать
            # умножение матрицы `A_part`на вектор `x_part`:
            Ax_part_temp = np.dot(A_part, x_part)
            # И просуммировать полученные вектора по всем
            # процессам своей строки сетки процессов, чтобы собрать
            # на каждом процессе первого столбца сетки процессов
            # свою часть вектора `Ax_part`, соответствующего
            # произведению `A.dot(x_{s})`:
            ...
            comm_row.Reduce([Ax_part_temp, M_part, MPI.DOUBLE],
                            ...
                            op=MPI.SUM, root=0)
            # Теперь на каждом процессе первого столбца сетки процессов
            # можно найти свою часть вектора `A.dot(x_{s}) - b`:
            if rank in procs_first_col:
                Axb_part = Ax_part - b_part
            else:
                # На остальных процессах подготовим "хранилище"
                # для копии этого вектора:
                Axb_part = np.empty(M_part, dtype=np.float64)
            # Теперь каждый процесс из первого столбца сетки процессов
            # раздаст копию своей части `Axb_part` всем процессам
            # в пределах своей строки сетки процессов:
            comm_row.Bcast([Axb_part, M_part, MPI.DOUBLE], root=0)
            ...
            # процессам своего столбца сетки процессов, чтобы собрать
            # на каждом процессе первой строки сетки процессов нужную
            # часть вектора `r_part`:
            ...
            # параллельно на каждом процессе первой строки сетки процессов
            ...
            # значение на каждом процессе первой строки сетки процессов!).
        ...
        # Сделаем это на процессах первой строки сетки процессов:
        if rank in procs_first_row:
        ...
        # Теперь каждый процесс из первой строки сетки процессов
        # раздаст копию своей части `p_part` всем процессам
        # в пределах своего столбца сетки процессов:
        ...
        # такого произведения. При этом нам достаточно собрать
        # эти части вектора только на процессах первой строки
        # сетки процессов (root=0):
        ...
        if rank in procs_first_row:
            ...
            # (префикс `All`) из первой строки сетки процессов:
            comm_row.Allreduce([dot_vec_temp, 1, MPI.DOUBLE],
                               [dot_vec, 1, MPI.DOUBLE], op=MPI.SUM)

            # Сейчас мы, наконец, можем обновить `x_part` на каждом
            # процессе из первой строки сетки процессов:
...
# строк элементов матрицы должны разбиться на `num_row` блоков
...
if rank == 0:
    ...
    # и записать её в свой массив `A_part`.
    ...
    for m in range(1, num_row):
        ...
        if rank in range(m*num_col, (m+1)*num_col):
            ...
            # строки `m` сетки процессов - для всех других процессах созданный выше
...
if rank in procs_first_col:
    ...
    # На всех остальных процессах поставим заглушки - они
    # понадобятся при вызове функции conjugate_gradient_method():
    b_part = None
...
# ВАЖНО: Реальные значения для `x_part` вернут только процессы из
# первой строки сетки процессов - проверим это (детали описаны
...
# присланными всеми процессами первой строки сетки процессов:
if rank in procs_first_row:
