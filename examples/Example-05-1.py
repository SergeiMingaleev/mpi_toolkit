...
def conjugate_gradient_method(A_part, b_part, x_part, N_part, M_part,
    ...
    # тогда как на всех остальных процессах уже выделена память для
    # нужного ему кусочка `b_part`. Наконец, на каждом процессе первой
    # строки сетки процессов есть свой кусочек `x_part` вектора `x`,
    # тогда как на всех остальных процессах уже выделена память для
    # нужного ему кусочка `x_part`.
    ...
    for s in range(1, N + 1):
        ...
        if s == 1:
            ...
            # процессам своего столбца сетки процессов, чтобы получить
        ...
        # Для минимизации объёма данных, которые мы будем должны 
...
# `displs_M`.
# Точно так же разберёмся, как именно N столбцов матрицы должны
...
# в терминах общего коммуникатора):
if rank in procs_first_row:
