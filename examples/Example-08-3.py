#------------------------------------------------------------------
# ИЗУЧИТЬ:
# -- Изучаем интерфейсы и работу методов отложенного запроса на
#    взаимодействие между процессами:
#    `cart.Send_init()`, `cart.Recv_init()`,
#    `MPI.Prequest.Startall()`, и `MPI.Request.Waitall()`.
...
#    Обратите внимание на использование методов отложенного
#    запроса на взаимодействие между процессами `MPI.Prequest.Startall()`
#    и `MPI.Request.Waitall()` в конце этих функций. Именно они
#    связывают между собой MPI процессы - и делают это чуть более
#    эффективным способом, чем мы делали в примере 8.2 (сравните!).

#------------------------------------------------------------------
#    Пример 8.3: Параллельное решение одномерного (1D)
...
# Реализация №3: Алгоритм в этом примере полностью совпадает с алгоритмом
#                в примере 8.2. Единственное отличие - вместо вызова
#                метода `comm.Sendrecv()` на каждой итерации по времени,
#                мы вызываем чуть более быстрые методы отложенного запроса
#                на взаимодействие между процессами `MPI.Prequest.Startall()`
#                и `MPI.Request.Waitall()`, предварительно (за пределами
#                итераций по времени!) инициализировав необходимые каналы
#                для коммуникаций методами `cart.Send_init()`
#                и `cart.Recv_init()`.
#
...
#
#------------------------------------------------------------------
# Новый функционал, который был использован в этом примере,
# детально обсуждается в лекции Д.В. Лукьяненко "Параллельные
# вычисления - 12. Тонкая оптимизация: отложенные запросы на
# взаимодействие":
# https://youtu.be/tR_sRjQxb6I?list=PLcsjsqLLSfNCxGJjuYNZRzeDIFQDQ9WvC
#
...
def slow_pde_solution():
    ...
    global requests, P, rank, M, u_part_aux, N_part_aux, \
           u_part_aux_left_send, u_part_aux_left_recv, \
           u_part_aux_right_send, u_part_aux_right_recv, \
...
# В этой реализации программы, мы будем использовать методы
# отложенного запроса на взаимодействие между MPI процессами.
# Для этого, сначала создадим на каждом процессе "хранилища"
# для пограничных точек (отдельно для посылки и для получения
# данных):
...
# И инициализируем каналы связи для отложенных запросов
# на взаимодействие:
...
if rank > 0:
    # Для левой границы на процессах от 1 до P-1:
    requests[0] = comm.Send_init([u_part_aux_left_send, 1, MPI.DOUBLE],
                                      dest=rank-1, tag=0)
    requests[1] = comm.Recv_init([u_part_aux_left_recv, 1, MPI.DOUBLE],
                                      source=rank-1, tag=0)
if rank < P-1:
    # И для правой границы на процессах от 0 до P-2:
    requests[2] = comm.Send_init([u_part_aux_right_send, 1, MPI.DOUBLE],
                                      dest=rank+1, tag=0)
    requests[3] = comm.Recv_init([u_part_aux_right_recv, 1, MPI.DOUBLE],
                                      source=rank+1, tag=0)
