...
# Считаем при этом, что p инициализирован нулём до начала
# итераций.
#
...
# решение x_{N}, которое для случая квадратной матрицы A будет
...
# Эта же матрица, но транспонированная, будет умножаться на
# векторы размером N, как делалось в примере 2.2.
...
# `conjugate_gradient_method()` - остальная часть кода занимается
...
def conjugate_gradient_method(A_part, b_part, x_part,
    ...
    # вспомогательный вектор `p`, нам понадобится знать `x` и `p`
    # полностью на каждом процессе:
    ...
    # Сразу инициализируем его нулём:
    ...
    for s in range(1, N + 1):
        ...
        if s == 1:
            # Сначала, на первой итерации s=1, нужно создать по особому
            # алгоритму кусочек `r_part` на каждом процессе - но для
            # этого нужно знать полный вектор `x`.

            # Сейчас каждый процесс хранит свой кусочек `x_part` вектора `x`.
            # Командой `comm.Allgatherv` мы можем собрать все кусочки вместе
            # в вектор `x` (суффикс `gatherv`!) и одновременно раздать его
            ...
            # полный вектор `r` с размером N, поместив эту часть в локальный
            # для каждого процесса вектор `r_temp`:
            ...
            # внутри себя полный вектор `r` - но не возвращая этот вектор,
            # сразу же разбросать его (суффикс `scatter`!) по всем процессам,
            # отдав каждому только нужную ему часть `r_part`:
            comm.Reduce_scatter([r_temp, N, MPI.DOUBLE],
                                [r_part, N_part, MPI.DOUBLE],
                                recvcounts=rcounts_N, op=MPI.SUM)
        else:
            # На всех последующих итерациях, кусочек `r_part` нужно просто
            # обновить.

            # Но для этого нам первым делом нужно знать скалярное
            # произведение векторов `p` и `q`, вычисленных на предыдущей
            # итерации:
            #    dot_pq = dot(p_{s-1}, q_{s-1})
            ...
            # `dot_vec`, где оно до сих пор ещё и хранится.

            # Воспользуемся им для обновления `r_part` на каждом процессе:
            ##--> r_{s} = r_{s-1} - q_{s-1} / dot_pq
...
if rank == 0:
    # Если вы знаете хорошее начальное приближение для `x`,
    # дайте его здесь!
...
# `A`, `b`, и `x` в виде `A_part`, `b_part`, и `x_part`, и возвращает
